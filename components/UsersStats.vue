...
import { CursorArrowRaysIcon, EnvelopeOpenIcon, UsersIcon, UserGroupIcon, UserMinusIcon, UserPlusIcon } from '@heroicons/vue/24/outline'

defineComponent({
  name: "UsersStats"
})
const stats = [
  { id: 1, name: 'Total Users', stat: '06', icon: UserGroupIcon, change: '5%', changeType: 'increase' },
  { id: 2, name: 'Active Users', stat: '04', icon: UserPlusIcon, change: '5.4%', changeType: 'increase' },
  { id: 3, name: 'Inactive Users', stat: '02', icon: UserMinusIcon, change: '3.2%', changeType: 'decrease' },
]
</script>
      ...
      <div v-for="item in stats" :key="item.id" class="relative overflow-hidden rounded-lg bg-white px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
        <dt>
          <div class="absolute rounded-md p-3"
               :class="['',
               (item.icon === UserGroupIcon ? 'bg-indigo-500 text-blue-800' :''),
               (item.icon === UserPlusIcon ? 'bg-green-500' : ''),
               (item.icon === UserMinusIcon ? 'bg-red-500' : '')
               ]"
          >
            <component :is="item.icon" class="h-6 w-6 text-white" aria-hidden="true" />
          </div>
          <p class="ml-16 truncate text-base font-normal text-gray-900">{{ item.name }}</p>
        </dt>
